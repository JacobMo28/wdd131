...
function compareFn(a,b) {
  if (a.price < b.productName) {
    return -1;
  } else if (a.price > b.productName) {
    return 1;
  }
 return 0;
}
...
function searchList(item){
    return item.name.toLowerCase().indcludes(query.toLowerCase());
}
...
let filteredTraits = animals.filter(function(item){
    ...
    trait.toLowerCase().include(queryTrait.toLowerCase()))
});
