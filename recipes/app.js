...
let cardContainer = document.querySelector(".media-card-container");
let button = document.querySelector('#eventForm');
...
function search(){
    ...
    let sortedRecipes = filterRecipe.sort(compareRecipe);
...
document.querySelector('form img').addEventListener('click', search);
...
function recipeTemplate(recipe) {
    return `<section class="media-card">
        ...
        <div class="media-card-content">
            <div class="media-card-tags">${tagTemplate(recipe.tags)}</div>
            <h3>${recipe.name}</h3>
...
function renderRecipe(recipe) {
    cardContainer.innerHTML += recipeTemplate(recipe);
}
